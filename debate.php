	  ...
	  $(function() {
		  function processNewComment(ev) {
			  ...
			  $.ajax({
					...
					url: 'comments.php',
					data: {
						id_debate: <?php echo $_GET['id_debate'];?>,
		  ...
		  function showsomething(data) {
			  ...
			  location.reload(data.id_debate);
		  }
		  $('#new-comment').on('submit', processNewComment);
	  });
...
<section class="pro">
	...
	<form id='new-comment' action="debate.php">
		...
		<br>
		<input class="add-user-comm" type="submit" value="Post Comment">
	  </form>
	  <div id="all_comments">
		  <?php
